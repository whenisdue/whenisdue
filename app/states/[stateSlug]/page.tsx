...
export default async function StatePage({ params }: PageProps) {
  ...
  console.log(`[D107.1] ${stateSlug.toUpperCase()} | Programs: ${matchedPrograms.join(", ") || "(none)"} | Rows: ${rawRules.length}`);

  const processedRules = validateRulesForState(stateSlug, rawRules.map(({ program, ...rule }) => rule));
          ...
          <div className="w-full lg:max-w-md space-y-6">
            {!canRender ? (
              ...
              <>
                ...
                {stateSlug === "new-york" && (
                  <NewYorkDecoder upstateRules={(processedRules as any).upstate} cityRules={(processedRules as any).city} />
                )}
              </>
            )}
