    ...
    <div className="bg-gray-900 border border-gray-800 p-8 rounded-3xl shadow-2xl max-w-md mx-auto mt-12 text-left">
      ...
      {activeTab === 'ssa' ? (
        <form onSubmit={handleSSASubmit} className="space-y-4 animate-in fade-in duration-300">
          <p className="text-gray-400 text-sm mb-4">Enter the day of the month you were born to find your exact 2026 SSA schedule.</p>
          <div>
            <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Birth Day (1-31)</label>
            <input 
              type="number" min="1" max="31" required value={birthDay} onChange={(e) => setBirthDay(e.target.value)}
              placeholder="e.g., 14"
          ...
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white py-4 rounded-xl font-black uppercase tracking-widest transition-all mt-2">
            Open 2026 Vault
          </button>
        </form>
      ) : (
        <form onSubmit={handleSNAPSubmit} className="space-y-4 animate-in fade-in duration-300">
          <p className="text-gray-400 text-sm mb-4">Find your exact monthly food stamp deposit day. (Privacy safe: only requires last digit).</p>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <select 
                  value={snapState} onChange={(e) => { setSnapState(e.target.value); setSnapResult(null); }}
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Case # Last Digit</label>
              <input 
                type="number" min="0" max="9" required value={caseDigit} onChange={(e) => setCaseDigit(e.target.value)}
                placeholder="e.g., 7"
          ...
          {snapResult ? (
            <div className="bg-green-900/20 border border-green-900 p-4 rounded-xl mt-4 animate-in zoom-in-95 duration-200">
              <div className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-1">Your Deposit Day</div>
              <div className="text-sm font-medium text-green-100">{snapResult}</div>
            </div>
          ) : (
            <button type="submit" className="w-full bg-green-700 hover:bg-green-600 text-white py-4 rounded-xl font-black uppercase tracking-widest transition-all mt-4">
              Calculate Day
            </button>
          )}
        </form>
